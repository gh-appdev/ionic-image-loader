...
var ImageLoader = (function () {
     ...
     */
    ImageLoader.prototype.getImagePath = function (imageUrl, priority) {
        var _this = this;
        ...
        return new Promise(function (resolve, reject) {
            var getImage = function () {
                _this.getCachedImagePath(imageUrl)
                    ...
                    _this.addItemToQueue(imageUrl, priority, resolve, reject);
                });
            };
            var check = function () {
                if (_this.isInit) {
     ...
     * @param reject
     */
    ImageLoader.prototype.addItemToQueue = function (imageUrl, priority, resolve, reject) {
        //If this image is already being processed add the promise to it.
        if (this.unCachedItemMap.has(imageUrl)) {
            var queueItem = this.unCachedItemMap.get(imageUrl);
            ...
            if (priority && this.queue.indexOf(imageUrl) > -1) {
                var index = this.queue.indexOf(imageUrl);
                if (index !== -1) {
    ...
    ImageLoader.prototype.processQueue = function () {
        ...
        var currentItemKey = this.priorityQueue.length > 0 ? this.priorityQueue.splice(0, 1)[0] : this.queue.splice(0, 1)[0];
        if (!this.unCachedItemMap.has(currentItemKey)) {
            return;
        }
        var currentItem = this.unCachedItemMap.get(currentItemKey);
        ...
        transfer.download(currentItem.imageUrl, localDir + "/" + tempPath, Boolean(this.config.fileTransferOptions.trustAllHosts), this.config.fileTransferOptions)
            ...
            for (var _i = 0, _a = currentItem.resolves; _i < _a.length; _i++) {
                var res = _a[_i];
                res(localUrl);
            }
            _this.unCachedItemMap.delete(currentItem.imageUrl);
            done();
        })
            .catch(function (e) {
            for (var _i = 0, _a = currentItem.rejects; _i < _a.length; _i++) {
                var rej = _a[_i];
                rej();
            }
            _this.unCachedItemMap.delete(currentItem.imageUrl);
